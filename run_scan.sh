...
echo ""
echo "[1/6] Scanning images (steghide + exiftool)..."
python3 scripts/scan_images.py

echo ""
echo "[2/6] Entropy analysis..."
python3 scripts/entropy_analysis.py

echo ""
echo "[3/6] Scanning audio and video files..."
python3 scripts/scan_audio_video.py

echo ""
echo "[4/6] Embedding algorithm analysis (clean vs stego)..."
python3 scripts/analyze_algorithms.py

echo ""
echo "[5/6] Bit-plane analysis (StegSolve headless)..."
python3 scripts/stegsolve_helper.py

echo ""
echo "[6/6] Running StegExpose on images..."
./run_stegexpose.sh
